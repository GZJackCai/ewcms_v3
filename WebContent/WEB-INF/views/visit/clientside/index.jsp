	...
	<script type="text/javascript">
		$(function() {
			...
			$('#tt').datagrid({
				...
				nowrap : true,
				striped : true,
				url : '${ctx}/visit/clientside/${typeValue}/table?startDate=' + $('#startDate').datebox('getValue') + '&endDate=' + $('#endDate').datebox('getValue'),
				columns:[[  
				        {field:'clientSideName',title:'名称',width:120,
				        	formatter : function(val, rec){
				        		return rec.clientSidePk.clientSideName;
				        	}	
				        }, 
				        {field:'rate',title:'比例',width:100},
				        {field:'clientSideCount',title:'PV数量',width:100},  
				        {field:'trend',title:'时间趋势',width:70,
				           	formatter : function(val, rec){	
				            	return '<a href="javascript:void(0)" style="text-decoration: none" onclick="openTrend(\'' + rec.clientSidePk.clientSideName + '\')">时间趋势</a>';
				            }
				        }
				]]  
			});
		...
		function showChart(){
			...
			$.post('${ctx}/visit/clientside/${typeValue}/report', parameter, function(result) {
		...
		function openTrend(value){
			var url = '${ctx}/visit/clientside/${typeValue}/trend/index?clientSideName=' + value + '&startDate=' + $('#startDate').datebox('getValue') + '&endDate=' + $('#endDate').datebox('getValue');
