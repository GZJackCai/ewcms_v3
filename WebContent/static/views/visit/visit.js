 ...
 * <script src="/js/visit.js" type="text/javascript"></script> <script>
 * if(window._ewcms_stat)_ewcms_stat("siteId=1&articleId=0&channelId=1&country=" +
 * remote_ip_info["country"] + "&province=" + remote_ip_info["province"] +
 * "&city=" + remote_ip_info["city"] + "&Dest=/visit"); </script>
 * 
 */
...
var _ewcms_stat = function(param) {
	...
	p["ce"] = _ewcms_stat.ce();
	p["je"] = _ewcms_stat.je();
	p["la"] = navigator.language ? navigator.language
...
_ewcms_stat.vq = function() {
	...
	var expires = new Date(new Date().getTime() + 365 * 10 * 24 * 60 * 60
			* 1000).toGMTString();
