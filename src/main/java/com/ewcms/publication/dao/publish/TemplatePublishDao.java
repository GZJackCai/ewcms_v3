...
@Repository
public class TemplatePublishDao implements TemplatePublishDaoable {
	private static final Logger logger = LoggerFactory.getLogger(TemplatePublishDao.class);
...
//	private static final String FIND_BODY_SQL = "SELECT t1.id,t1.uniquepath,t2.tplentity FROM site_template t1,site_templateentity t2 WHERE t1.tplentityid = t2.id AND t1.uniquepath= :path";
	private static final String FIND_BODY_SQL = "SELECT t1.id,t1.uniquepath,t2.tplentity FROM site_template t1 LEFT JOIN site_templateentity t2 ON t1.tplentityid = t2.id WHERE t1.uniquepath= :path";
