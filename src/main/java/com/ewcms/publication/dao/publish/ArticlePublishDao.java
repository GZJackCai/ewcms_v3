...
@Repository
public class ArticlePublishDao implements ArticlePublishDaoable{
	private static final String PRERELEASE_STATUS = com.ewcms.content.document.model.Article.Status.PRERELEASE.name();
	private static final String RELEASE_STATUS = com.ewcms.content.document.model.Article.Status.RELEASE.name();
	
	private static final String FIND_PRE_PUBLISH_BY_IDS_SQL = "SELECT t1.*, t2.page, t2.detail FROM content_article t1 LEFT JOIN content_content t2 ON t1.id = t2.article_id "
			+ " WHERE t1.id IN (:ids) AND t1.is_delete = FALSE ";
	...
	static class ArticleMapper implements RowMapper<Article>{
		...
		public Article mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			a.setComment(rs.getBoolean("is_comment"));
