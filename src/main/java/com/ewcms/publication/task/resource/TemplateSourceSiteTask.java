...
public class TemplateSourceSiteTask extends TaskBase{
	...
	public TemplateSourceSiteTask(Site site, String tempRoot, 
			boolean again, TemplateSourcePublishDaoable templateSourcePublishDao) {
		
		this(null,false,site,tempRoot,again,templateSourcePublishDao);
	}
	
	public TemplateSourceSiteTask(String parentId, Site site, 
			String tempRoot, boolean again, TemplateSourcePublishDaoable templateSourcePublishDao) {
		
		this(parentId,true,site,tempRoot,again,templateSourcePublishDao);
	...
	protected void doHandler(List<DepTask> tasks, int count, int index, int batchSize) {
		List<TemplateSource> sources = templateSourcePublishDao.findPublish(site.getId(), again, startId, batchSize);
	...
	protected int totalCount() {
		return templateSourcePublishDao.findPublishCount(site.getId(), again);
	}
}
