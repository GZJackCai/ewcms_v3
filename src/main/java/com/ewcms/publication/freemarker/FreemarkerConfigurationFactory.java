...
public class FreemarkerConfigurationFactory implements FactoryBean<Configuration>, InitializingBean{
	...
	private TemplateLoader[] initTemplateLoader() {
		return new TemplateLoader[]{new DatabaseTemplateLoader(templatePublishDao)};
	}
	...
	private Map<String, Object> initFreemarkerVariables() {
		...
		variables.put("article", new ArticleDirective(articlePublishDao));
		...
		variables.put("article_list", new ArticleListDirective(channelPublishDao, articlePublishDao));
		variables.put("channel_list", new ChannelListDirective(channelPublishDao));
		variables.put("position", new PositionDirective(channelPublishDao));
		variables.put("index", new IndexDirective());
		variables.put("include", new IncludeDirective(channelPublishDao,templatePublishDao));
	...
	public void setTemplateService(TemplatePublishDaoable service) {
		this.templatePublishDao = service;
	}

	public void setChannelService(ChannelPublishDaoable channelService) {
		this.channelPublishDao = channelService;
	}

	public void setArticleService(ArticlePublishDaoable articleService) {
		this.articlePublishDao = articleService;
	}
