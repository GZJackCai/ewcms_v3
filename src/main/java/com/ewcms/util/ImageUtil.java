...
public class ImageUtil {
	 ...
	 * @return 压缩是否成功
	 */
	public static Boolean compression(MultipartFile sourcePath,
			String targetPath, int width, int hight) {

		try {
			// File sourceFile = new File(sourcePath);
			File targetFile = new File(targetPath);

			String format = getFormatName(sourcePath);
			logger.info("Image Format is {}", format);

			BufferedImage srcImage = ImageIO.read(sourcePath.getInputStream());
			if (width > 0 || hight > 0) {
	...
	public static Boolean compression(String sourcePath, String targetPath,
			int width, int hight) {
	 ...
	 * @throws IOException
	 */
	public static String getFormatName(MultipartFile sourcePath)
			throws IOException {
		ImageInputStream iis = ImageIO.createImageInputStream(sourcePath
				.getInputStream());
