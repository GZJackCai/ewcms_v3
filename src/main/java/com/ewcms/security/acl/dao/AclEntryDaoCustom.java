...
public interface AclEntryDaoCustom {
	...
	@Query("select max(e.aceOrder) from AclEntry e left join e.aclIdEntity i where i.objectId=?1")
	Long findByMaxOrder(Long idEntityId);
	
	@Query("select e from AclEntry e left join e.aclIdEntity i where i.objectId=?1")
	...
	@Query("select max(e.mask) from AclEntry e left join e.aclIdEntity i left join e.aclSid s left join i.aclClass c where i.objectId=?1 and c.className=?2 and (s.sid=?3 or s.sid in (?4)) ")
