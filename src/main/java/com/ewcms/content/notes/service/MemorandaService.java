...
public class MemorandaService {
	...
	public StringBuffer getInitCalendarToHtml(final int year, final int month, String servletContentName) {
		...
		sb.append(generatorFirstWeekHtml(firstDayOfMonth, servletContentName).toString());
		sb.append(generatorMiddleWeekHtml(week, servletContentName).toString());
		sb.append(generatorLastWeekHtml(days,firstDayOfMonth, servletContentName).toString());
	...
	private StringBuffer generatorJs(){
        ...
        sb.append("       }\n");
        sb.append("       $.post(dropUrl + '/' + memoId + '_' + $('#year').val() + '_' + $('#month').val() + '_' + dropDay,{},function(data){\n");
        sb.append("         if (!data){\n");
        sb.append("         }\n");
        sb.append("       });\n");
        sb.append("     }\n");
	...
	private StringBuffer generatorContentHtml(int year, int month, int day, String position, String servletContentName){
		StringBuffer sb = new StringBuffer();
		...
		for (Memoranda memo : memos){
			...
			String clock = "";
			if (memo.getWarn()){
				clock = "<img id='img_clock_" + memo.getId() + "' src='" + servletContentName + "/static/image/notes/clock.png' width='13px' height='13px' align='bottom'/>";
			}
			if (title.length() > 12){
				title = title.substring(0, 9) + "..."; 
			}
			memoSb.append("<div id='div_notes_memo_" + memo.getId() + "' class='a_notes_value'><span id='title_" + memo.getId() + "' class='span_title'><a class='a_title' id='a_title_" + memo.getId() + "' onclick='_notesIndex.editNotes(" + memo.getId() + ");' style='cursor:pointer;' href='javascript:void(0);' title='" + memo.getTitle() + "'>" + title + "</a></span>" + clock + "<div></div></div>\n");
		}
		
		sb.append("  <td id='td_notes_" + position + day + "'>\n");
		sb.append("    <table width='100%' cellspacing='0' cellpadding='0' border='0'");
		if (position.equals("")){
			sb.append(" ondblclick='_notesIndex.addNotes(" + day + ");'");
		}
		sb.append(">\n");
		sb.append("      <tr>\n");
	...
	private StringBuffer generatorFirstWeekHtml(final int firstDayOfMonth, String servletContentName) {
		StringBuffer sb = new StringBuffer();
		...
		for (int i = 1; i < firstDayOfMonth; i++){
			sb.append(generatorContentHtml(calendar.get(Calendar.YEAR), prevMonth, beginDay, POSITION_PREV, servletContentName).toString());
			beginDay++;
		}
		for (int i = firstDayOfMonth; i<=7;i++){
			sb.append(generatorContentHtml(selYear, selMonth, dayCount, "", servletContentName).toString());
			dayCount++;
		}
		sb.append("</tr>\n");
	...
	private StringBuffer generatorMiddleWeekHtml(final int week, String servletContentName){
		StringBuffer sb = new StringBuffer();
	...
	private StringBuffer generatorLastWeekHtml(final int days, final int firstDayOfMonth, String servletContentName){
		StringBuffer sb = new StringBuffer();
		...
		for (int i = 1; i <= blankDay; i++){
			sb.append(generatorContentHtml(calendar.get(Calendar.YEAR), nextMonth, i, POSITION_NEXT, servletContentName).toString());
		}
		sb.append("</tr>\n");
	...
	public Long addMemoranda(Memoranda memoranda, Integer year, Integer month, Integer day) {
		memoranda.setUserName(EwcmsContextUtil.getLoginName());
	...
	public void delMemoranda(Long memorandaId) {
		memorandaDao.delete(memorandaId);
	}

	public Memoranda findMemoranda(Long memorandaId) {
		return memorandaDao.findOne(memorandaId);
	}
	...
	private List<Memoranda> findMemorandaByDate(Integer year, Integer month, Integer day) {
		...
		return memorandaDao.findMemorandaByDate(beginDate, endDate, EwcmsContextUtil.getLoginName());
	}
	...
	public void updMemoranda(Long memorandaId, Integer year, Integer month, Integer day) {
		...
		Memoranda memoranda = memorandaDao.findOne(memorandaId);
		memoranda.setNoteDate(calendar.getTime());
	...
	public List<Memoranda> findMemorandaByUserName() {
		return memorandaDao.findMemorandaByUserName(EwcmsContextUtil.getLoginName());
	}
	
	public Map<String, Object> search(QueryParameter params){
		return SearchMain.search(params, "IN_id", Long.class, memorandaDao, Memoranda.class);
	}
}
