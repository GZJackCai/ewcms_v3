...
<html>
  ...
  <body>
	...
	<form:form id="inputForm" action="${ctx}/scheduling/job/report/save" method="post" modelAttribute="pageDisplay" class="form-horizontal">
	  <table class="formtable" align="center">
		...
		<tr>
		  ...
		  <td width="10%">报表名称：</td>
		  <td>${pageDisplay.reportName}
		    <c:if test="${pageDisplay.reportType=='text'}">(文字报表)</c:if>
		    <c:if test="${pageDisplay.reportType=='chart'}">(图形报表)</c:if>
		    <input type="hidden" id="reportType" name="reportType" value="${pageDisplay.reportType}"/>
		  </td>
		</tr>
		<tr>
		  <td>说明：</td>
		...
		<c:if test="${not empty pageDisplay.reportType && not empty pageDisplay.pageShowParams}">
		<tr>
		  ...
		  <td colspan="3">
		    <c:if test="${pageShowParam.type == 'TEXT' || pageShowParam.type == 'BOOLEAN' || pageShowParam.type == 'SQL' || pageShowParam.type == 'SESSION'}">
		  	...
		  	<c:if test="${pageShowParam.type == 'CHECK'}">
		  	  <form:checkboxes items="${pageShowParam.value}" path="${paramShowParam.enName}" onclick="_jobReportEdit.checkBoxValue('${paramShowParam.enName}')"/>
		  	  <input type="hidden" name="${pageShowParam.enName}"/>
		  	</c:if>
		  	<c:if test="${pageShowParam.type == 'DATE'}">
		  	  <input type="text" name="${pageShowParam.enName}" value="${pageShowParam.defaultValue}" class="easyui-datetimebox" data-options="showSeconds:false"/>
		  	</c:if>
		  </td>
		</tr>
		</c:forEach>
	  ...
	  <input type="hidden" id="reportId" name="reportId" value="${pageDisplay.reportId}"/>
	</form:form>
	<%@ include file="/WEB-INF/views/jspf/import-js.jspf" %>
	<script type="text/javascript" src="${ctx}/static/views/scheduling/jobinfo/editreport.js"></script>
    <script type="text/javascript">
		if ('${close}' == 'true'){
	  		parent.$('#edit-window').window('close');
	  	}
		var _jobReportEdit = new JobReportEdit();
		$(function(){
			_jobReportEdit.init();
		});
    </script>
