	...
	<script type="text/javascript">
		$(function() {
			...
			$('#tt').datagrid({
				...
				nowrap : true,
				striped : true,
				url : '${ctx}/visit/clickrate/searchengine/table?startDate=' + $('#startDate').datebox('getValue') + '&endDate=' + $('#endDate').datebox('getValue'),
			    columns:[[  
			            {field:'engineName',title:'域名',hidden:true,
			            	formatter : function(val, rec){
			            		return rec.searchEnginePk.engineName;
			            	}	
			            },
			            {field:'engineValue',title:'名称',width:120,
			            	formatter : function(val, rec){
			            		return rec.searchEnginePk.engineValue;
			            	}	
			            }, 
			            {field:'engineCount',title:'UV',width:100},
			            {field:'rate',title:'比例',width:100},  
			            {field:'trend',title:'时间趋势',width:70,
			            	formatter : function(val, rec){	
			            		return '<a href="javascript:void(0)" style="text-decoration: none" onclick="openTrend(\'' + rec.searchEnginePk.engineName + '\')">时间趋势</a>';
		            	}
			        }
				]]  
			});
		...
		function showChart(){
			...
			$.post('${ctx}/visit/clickrate/searchengine/report', parameter, function(result) {
		...
		function openTrend(value){
			var url = '${ctx}/visit/clickrate/searchengine/trend/index?searchEngineName=' + value + '&startDate=' + $('#startDate').datebox('getValue') + '&endDate=' + $('#endDate').datebox('getValue');
